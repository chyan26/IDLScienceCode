...
PRO IRACCATALOG, cat
   ...
   hd=headfits(conf.fitspath+'s233ir_h2.fits')
   
   factor=0.033846
   
   RESTORE,conf.catpath+'irac_i1_080509.db'
   ...
   RESTORE,conf.catpath+'irac_i2_080509.db'
   ;[ra,dec,rnd,shp,aper,insky,outsky,mag,magerr,sky,skyerr,user_status]
   id=indgen(n_elements(pv_dat[0,*]))
   ra=reform(pv_dat[0,*])
   dec=reform(pv_dat[1,*])
   ...
   RESTORE,conf.catpath+'irac_i3_080509.db'
   ;[ra,dec,rnd,shp,aper,insky,outsky,mag,magerr,sky,skyerr,user_status]
   id=indgen(n_elements(pv_dat[0,*]))
   ra=reform(pv_dat[0,*])
   dec=reform(pv_dat[1,*])
   ...
   RESTORE,conf.catpath+'irac_i4_080509.db'
   ;[ra,dec,rnd,shp,aper,insky,outsky,mag,magerr,sky,skyerr,user_status]
   id=indgen(n_elements(pv_dat[0,*]))
   ra=reform(pv_dat[0,*])
   dec=reform(pv_dat[1,*])
...
PRO DUMPMERGECAT, cat	
   ...
   loadconfig
	...
	close,1
	openw,1,conf.sedpath+'source.cat'
...
;
;  This subroutine is used to produce the region file for DS9 to make the 
;    position of YSOs.
;
PRO YSOREGION, final
   COMMON share,conf
   ...
   ; Set this keyword to 1 for plotting YSO from SED fitter. 
   plotirac=0
   
   ; Selecting CTTS stars from the input catalog
   ...
   ; Reading the YSO catalog
   readcol,conf.sedpath+'output_yso_good.txt',format='(A30,F9.5,F9.5)',$
    id,ra,dec
   ...
   ;print,x
   ; This is the stars that JHK > IRAC and show V-shape in IRAC bands
   type1=[230,237,273,2,382,445,478,518,537,121,153,205,209] 
   
   ; stars JHK > IRAC without V-shape in IRAC band
   type2=[61,622,625,82,213,240,245,289,299,115,131,195,196,36,450,463] 
   
   ; stars that JHK < IRAC
   type3=[293,300,159,189]
   
   ; Excess in IRAC
   type4=[248,280,129,203,345,417,430]
   print,n_elements(type1),n_elements(type2),n_elements(type3),n_elements(type4)
   
   regfile = conf.regpath+'ysoregion_sed.reg'
   
   openw,fileunit, regfile, /get_lun 
   
   ind=where(final.mj lt -500 and final.mh ge -500 and final.mk ge -500 and $
      final.mh-final.mk ge 1.5, count)
   if count ne 0 then begin
      for i=0,count-1 do begin
        print,final.mj[ind[i]],final.mh[ind[i]],final.mk[ind[i]],$
          final.mh[ind[i]]-final.mk[ind[i]]
        printf,fileunit,'point('+strcompress(string(final.x[ind[i]]),/remove_all)$
            +','+strcompress(string(final.y[ind[i]]),/remove_all)+') #point=box color=green'
     endfor    
   end
   
   ind=where(ctts ne 0, count)
   if count ne 0 then begin
     for i=0,count-1 do begin
        printf,fileunit,'point('+strcompress(string(final.x[ind[i]]),/remove_all)$
            +','+strcompress(string(final.y[ind[i]]),/remove_all)+') #point=cross color=red'
     endfor 
   endif
   
   
   ; Set a keyword to control the plot of YSOs.
   if plotirac eq 1 then begin
   
     for i=0,n_elements(id)-1 do begin
       ind=where(id[i] eq type1, count)
       if count ne 0 then begin
          printf,fileunit,'point('+strcompress(string(x[i]),/remove_all)+','+$
                 strcompress(string(y[i]),/remove_all)+') # point=diamond'
       endif
       
       ind=where(id[i] eq type2, count)
       if count ne 0 then begin
          printf,fileunit,'point('+strcompress(string(x[i]),/remove_all)+','+$
                 strcompress(string(y[i]),/remove_all)+') # point=circle'
       endif     
       
       ind=where(id[i] eq type3, count)
       if count ne 0 then begin
          printf,fileunit,'point('+strcompress(string(x[i]),/remove_all)+','+$
                 strcompress(string(y[i]),/remove_all)+') # color=white point=cross'
       endif     
       
       ind=where(id[i] eq type4, count)
       if count ne 0 then begin
          printf,fileunit,'point('+strcompress(string(x[i]),/remove_all)+','+$
                 strcompress(string(y[i]),/remove_all)+') # color=red point=box'
       endif     
