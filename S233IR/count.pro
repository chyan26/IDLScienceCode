...
PRO NORSTARCOUNT, cat, Ps=ps
   ...
   if keyword_set(PS) then begin 
      set_plot,'ps'
      device,filename=conf.pspath+'norstarcount.eps',$
   ...
   readcol,conf.catpath+'porras.cat',id,ra,dec,mj,mjerr,mh,mherr,mk,mkerr
...
PRO CLUSTERCOUNT, cat, porrascat, Ps=ps
   ...
   if keyword_set(PS) then begin 
      set_plot,'ps'
      device,filename=conf.pspath+'clustercount.eps',$
