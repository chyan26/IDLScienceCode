PRO ALMA_DETECT_SKY2, START_FIELD, END_FIELD, SIGMA, CONFIG
	...
	all_sigma=series(start_sigma, end_sigma, n_sigma)
	table=fltarr(n_sigma,5)	
	result=fltarr(n_sigma,5)
	
	for config=1,3 do begin
	table(*)=0
	result(*)=0
		for i=start_field, end_field do begin
			...
			for j=0, n_elements(all_sigma)-1 do begin	
				count(*)=0
				rate(*)=0
				sigma=all_sigma(j)
		...
		for j=0, n_elements(all_sigma)-1 do begin
			...
			printf,1,'Total No. of source(s) inputed',result(j,0)-100*(end_field-start_field+1)
			printf,1,'No. of source(s) with f > 3-sigma:',result(j,1)
			printf,1,'No. of source(s) detected by FIND:',result(j,2)
			printf,1,'No. of source(s) f > 3-sigma and detected:',result(j,3)
			printf,1,'No. of source(s) f < 3-sigma but detected:',result(j,4)
			close,1	
		endfor
...
PRO REJECT_FALSE, X, Y, F, X_F, Y_F, F_F, X_D, Y_D, FLUX, RMS
	...
	for i=0, n_elements(x)-1 do begin
		...
		if min(r) lt 36 and min(abs(f(i)-f_f(ind))) lt 3*rms then begin
