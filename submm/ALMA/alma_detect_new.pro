PRO ALMA_DETECT_NEW, START_FIELD, END_FIELD, SIGMA, CONFIG
	...
	for i=start_field, end_field do begin
		...
		bmaj=sxpar(headfits(fitsfile),'BMAJ')*3600/0.07
                bmin=sxpar(headfits(fitsfile),'BMIN')*3600/0.07
                fwhm=sqrt(bmaj*bmin)
		print,fwhm
		!x.charsize=2		
		!y.charsize=2	
		...
		th_image_cont,im,level=[3*rms,4*rms,5*rms],xrange=[-9,9]$
			,/aspect,yrange=[-9,9],/nobar,crange=[1*rms,15*rms]
		rad=[bmaj*0.07,bmin*0.07]
		ellipse,center=-8.5+rad,radii=rad,/fill

		oplot_source,flux,x,y,sigma,rms
		xyouts,-8,8,'('+im2(config-1)+')',charsize=1.3,/data

		data=detect_count(im,fwhm,sigma,rms,i,config)
		if data(0) eq -1 then begin
			...
			dd=d_rate(flux,x,y,data(0,*),data(1,*),data(2,*),fwhm,sigma,rms)
...
;
	...
	;th_image_cont,im,level=[2*rms,3*rms],xrange=[-9,9]$
	;	,/aspect,yrange=[-9,9],/nobar,ct=0$
	...
	;oplot,(x_a*0.07)-9,(y_a*0.07)-9,psym=4,symsize=2
;
...
;window,1
;wset,1
	;th_image_cont,bg_image,level=[2*rms,3*rms],xrange=[-9,9]$
	;	,/aspect,yrange=[-9,9],/nobar,ct=0$
	;	,crange=[0.1*rms,5*rms]
;
	auto_find_sma,bb,fwhm,rms/1.5,sigma,x_f,y_f,ff
;	oplot,x_f*0.07-9,y_f*0.07-9,psym=6
;wset,0
	...
	if keyword_set(x_d) eq 0 then begin
		...
		x_d=x_d*0.07-9
		y_d=y_d*0.07-9
		;oplot,x_d,y_d,psym=5
		;oplot,x_d,y_d,psym=5,symsize=5
...
FUNCTION D_RATE, FLUX, X, Y, F_D, X_D, Y_D, FWHM, RMS, SIGMA, COUNT
	...
	if c ne 0 then begin
		for i=0,n_elements(x_d)-1 do begin
			...
			if sqrt(min(r))/0.07 lt fwhm then begin
				dd=dd+1
			endif else begin
				dd=dd+0
			endelse
...
PRO OPLOT_SOURCE, FLUX, X, Y, SIGMA, RMS
	...
	if c ne 1 then begin
		plots,x(index),y(index),psym=1,symsize=10
	endif else begin
		plots,x(index),y(index),psym=1,symsize=10
	endelse
	...
	if n_elements(ind_c) ne 1 then begin
		oplot,x(ind_c),y(ind_c),psym=1,symsize=5
	endif else begin
		plots,x(ind_c),y(ind_c),psym=1,symsize=5
	endelse
	final:
	no=c
END
...
PRO REJECT_FALSE, X, Y, F, X_F, Y_F, F_F, X_D, Y_D, FLUX, RMS
	...
	for i=0, n_elements(x)-1 do begin
		...
		if sqrt(min(r)) lt 5 and min(abs(f(i)-f_f(ind))) lt 2*rms then begin
