pro fit_scuba_obs2
	...
	s=10^(((findgen(200)+1)/20)-8)
	n1=scott_sn(1.1d4,1.79,0.0,3.13,s)
	nn=fltarr(n_elements(n1))
	for i=0, n_elements(n1)-3 do begin
		nn(i)= int_tabulated(s(i+1:n_elements(s)-1),n1(i+1:n_elements(n1)-1),/sort)
	endfor
	print,nn
	oplot,s,nn
	;oplot,s,n1
...
PRO scuba_plot,x,y,psym_id
	...
	plot,x(index),y(index),psym=psym_id(min(index)),/xlog,xrange=[0.1,100],xstyle=1,yrange=[1,1.5d4]$
	,/ylog
	for i=4,8 do begin
