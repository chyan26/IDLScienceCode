PRO SMA_DETECT_NEW, START_FIELD, END_FIELD, SIGMA, CONFIG
	...
	result(*)=0
		for i=start_field, end_field do begin
			...
			bmaj=sxpar(headfits(fitsfile),'BMAJ')*3600/0.14
			bmin=sxpar(headfits(fitsfile),'BMIN')*3600/0.14
			fwhm=sqrt(bmaj*bmin)		
...
;wset,0
	...
	if keyword_set(x_d) eq 0 then begin
		...
		oplot,x_d,y_d,psym=5,symsize=3
...
FUNCTION D_RATE, FLUX, X, Y, F_D, X_D, Y_D, FWHM, RMS, SIGMA, COUNT
	...
	if c ne 0 then begin
		for i=0,n_elements(x_d)-1 do begin
			...
			if sqrt(min(r)) lt fwhm then begin
				dd=dd+1
			endif else begin
				dd=dd+0
			endelse
...
PRO OPLOT_SOURCE, FLUX, X, Y, SIGMA, RMS
	...
	if c ne 1 then begin
		plots,x(index),y(index),psym=6,symsize=3
	endif else begin
		plots,x(index),y(index),psym=6,symsize=3
	endelse
	...
	if n_elements(ind_c) ne 1 then begin
		oplot,x(ind_c),y(ind_c),psym=1,symsize=3
	endif else begin
		plots,x(ind_c),y(ind_c),psym=1,symsize=3
	endelse
	final:
	no=c
END
