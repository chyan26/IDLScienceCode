	...
	for j=0, n_elements(image.(1)(*,0,0))-1 do begin
      ;!p.multi=0
		...
		for i=1, n_tags(image)-1 do begin
            ...
            th_image_cont,(peak+width)-im,/nocont,/nobar,xrange=[-(conf.xsize/2),(conf.xsize/2)],$
				yrange=[-(conf.ysize/2),conf.ysize/2],crange=[peak-width,peak+width]
            multiplot
			endelse 
		endfor
		multiplot,/reset
...
PRO EXTRACTIMAGE_WIRCAMRADEC_2, ind
