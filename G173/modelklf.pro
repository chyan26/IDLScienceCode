...
PRO KLFDATA, mass, age, mk, h, xh
  ...
  ind=where(rm1 ge max(dm97_iso.mass) and rm1 le max(sdf_iso.mass))
  ...
  ; age = 0.1 is a special case
  if age eq 0.1 then begin
    ind=where(h eq 0)
    ii=where(h[ind-1] ne 0 and h[ind+1] ne 0)
    h[ind[ii]]=0.5*(h[ind[ii]+1]+h[ind[ii]-1])
  endif
