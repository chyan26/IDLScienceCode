...
;  iImage,rot(mom0,pa,CUBIC=-0.5), Margin=0.01, /Axes, CTIndex=4, /Brewer, /Reverse,/fit_to_view
  pa=0
  sigmat = 0.25                      ;Jy/beam one sigma value of image ;1.12
...
;  PRINT, 'GETIM: image pv min  = ', MIN(image_pv),' Jy/beam'
  PRINT, 'GETIM: image all max = ', MAX(image),' '+funit
  PRINT, 'GETIM: image all min = ', MIN(image),' '+funit
  PRINT, 'GETIM: image all std = ', STDDEV(image),' '+funit
  PRINT, 'GETIM: image rot max = ', MAX(image_rot),' '+funit
  PRINT, 'GETIM: image rot min = ', MIN(image_rot),' '+funit
  PRINT, 'GETIM: image pv max  = ', MAX(image_pv),' '+funit
  PRINT, 'GETIM: image pv min  = ', MIN(image_pv),' '+funit
  PRINT, 'GETIM: image pv std  = ', STDDEV(image_pv),' '+funit
