...
;	ythick=5.0
  ...
  if keyword_set(disp) then begin
   plot,cmh-cmk,cmj-cmh,psym=4,xrange=[-0.5,2.5],yrange=[0,3],$
	...
	corrcat={x:cat.x,y:cat.y,mj:mj,mh:mh,$
...
PRO AVGEXT, cat
   ...
   ind=where(cat.group eq 1 and cat.av ge 0.0, count)
   print,'HCN group=',mean(cat.av[ind]),' +/-',stddev(cat.av[ind])
  
   ind=where(cat.group eq 2 and cat.av ge 0, count)
   print,'Continuum= ',mean(cat.av[ind]),' +/-',stddev(cat.av[ind])
